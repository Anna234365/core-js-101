...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str.charCodeAt(i) > 64 && str.charCodeAt(i) < 78) {
      ...
      newStr += String.fromCharCode((str.charCodeAt(i) + 13 + 64) % 90);
    } else if (str.charCodeAt(i) > 96 && str.charCodeAt(i) < 110) {
      newStr += String.fromCharCode(str.charCodeAt(i) + 13);
    } else if (str.charCodeAt(i) > 109 && str.charCodeAt(i) < 123) {
      newStr += String.fromCharCode((str.charCodeAt(i) + 13 + 96) % 122);
    } else {
      newStr += str[i];
    }
  }
...
function getCardId(value) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i] === value.slice(0, -1)) {
      q = i;
    }
  }
  for (let i = 0; i < arr2.length; i += 1) {
    if (arr2[i] === value[value.length - 1]) {
      w = i * 13;
    }
  }
  return q + w;
